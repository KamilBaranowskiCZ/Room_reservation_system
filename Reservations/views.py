...
class RoomListView(View):
    def get(self, request):
        ...
        return render(request, "rooms_list.html", context={"rooms_list": rooms_list, "rooms": rooms})
...
class AddRoomView(View):
    ...
    def post(self, request):
        ...
        try:
            ConferenceRoom.objects.create(name=name, capacity=capacity, has_projector=projector)
        except:
            return render(request, "add_conference_room.html", context={"error": "Sala o podanej nazwie już istnieje"})
        return redirect("/")
...
class EditRoomView(View):
    ...
    def post(self, request, room_id):
        ...
        try:
            ...
            return render(request, "edit_room.html", context={"error": "Sala o podanej nazwie już istnieje"})
        return redirect("/")
...
class RoomReservationView(View):
    ...
    def post(self, request, room_id):
        ...
        if Reservation.objects.filter(room_id=room_id, date=date):
            return render(request, "reservation.html", context={"room": room, "reservations": reservations, "error": "Sala jest już zarezerwowana!"})
        if date < str(datetime.date.today()):
            return render(request, "reservation.html", context={"room": room, "reservations": reservations, "error": "Data jest z przeszłości!"})
...
class SearchView(View):
    def get(self, request):
        ...
        for room in rooms:
            reservation_dates = [reservation.date for reservation in room.reservation_set.all()]
            room.reserved = datetime.date.today() in reservation_dates
        return render(request, "rooms_list.html", context={"rooms": rooms, "date": datetime.date.today()})